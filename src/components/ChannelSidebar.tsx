  ...
  return (
    <div className={`${showMobileSidebar ? "translate-x-0 left-[72px]" : "-translate-x-full left-0"} lg:translate-x-0 lg:inset-y-auto lg:left-0 fixed inset-y-0 lg:relative z-50 lg:z-auto bg-surface flex lg:flex flex-col shrink-0 h-full transition-all duration-200 w-60`}>
      {viewMode !== "server" ? (
        <>
          <div className="h-12 px-3 flex items-center gap-1 border-b border-border-strong shadow-sm shrink-0">
            ...
            <button onClick={() => setSideTab("friends")} className={`flex-1 py-1.5 rounded text-[13px] font-semibold transition-colors flex items-center justify-center gap-1.5 ${sideTab === "friends" ? "bg-surface-active text-foreground" : "text-zinc-400 hover:text-foreground"}`}>
              Amigos
              {incomingRequests.length > 0 && <span className="min-w-4 h-4 px-1 rounded-full bg-danger text-white text-[10px] font-bold inline-flex items-center justify-center">{incomingRequests.length > 9 ? "9+" : incomingRequests.length}</span>}
            </button>
          ...
          <div className="p-2">
            <div className="relative mb-2">
              ...
              <input value={dmSearch} onChange={(e) => setDmSearch(e.target.value)} placeholder="Buscar DM" className="w-full bg-input-bg rounded pl-7 pr-2 py-1.5 text-sm focus:outline-none placeholder:text-zinc-400" />
            </div>
          </div>
          <div className="flex-1 overflow-y-auto p-2 space-y-1">
            {filteredDMs.length === 0 ? (
              <div className="text-center py-8 px-4">
                <div className="w-14 h-14 rounded-full bg-surface-active flex items-center justify-center mx-auto mb-3">
              ...
              <button key={dm.id} onClick={() => { setSelectedDM(dm.id); setViewMode("dm"); }} className={`w-full flex items-center gap-3 px-2 py-2 rounded text-left ${selectedDM === dm.id ? "bg-surface-active text-foreground" : "text-zinc-400 hover:bg-surface-hover hover:text-foreground"}`}>
                ...
                <div className={`w-2 h-2 rounded-full ${onlineMembers.some((m) => m.id === dm.otherUser?.id) ? "bg-success" : "bg-zinc-600"}`} />
              </button>
            ))}
              ...
              <div className="mt-2 space-y-1">
                  {onlineMembers.slice(0, 5).map((m) => (
                    <button key={m.id} onClick={() => { setNewDMUsername(m.username); useModalStore.getState().openModal("showNewDMModal"); }} className="w-full flex items-center gap-2 px-2 py-1 rounded hover:bg-surface-hover text-left">
                      <Avatar src={m.avatar} name={m.username} className="w-6 h-6 rounded-full bg-surface-active text-xs" />
          ...
          <div className="flex-1 overflow-y-auto p-2 space-y-2">
            {activeVoice.length > 0 && (
              <div>
                ...
                {activeVoice.map((g) => (
                  <div key={g.channelId} className="mb-1.5 rounded-lg bg-surface p-2">
                    <div className="flex items-center gap-2">
                      <Volume2 className="w-3.5 h-3.5 text-success shrink-0" />
                      <div className="flex-1 min-w-0">
                        <div className="text-xs font-semibold text-foreground truncate">{g.channelName}</div>
                        <div className="text-[10px] text-zinc-400 truncate">{g.serverName} • {g.users.length} em voz</div>
                      </div>
                      <button onClick={() => onJoinVoice(g.serverId, g.channelId)} className="px-2 py-1 rounded bg-success hover:bg-success-hover text-white text-[11px] font-bold shrink-0">Entrar</button>
                    </div>
                    <div className="mt-1.5 flex items-center">
                      {g.users.slice(0, 5).map((u) => (
                        <button key={u.id} onClick={() => onViewProfile(u.id)} title={u.username} className="-ml-1 first:ml-0">
                          <Avatar src={u.avatar} name={u.username} className="w-6 h-6 rounded-full border-2 border-surface bg-surface-active text-[10px]" />
                        </button>
                      ))}
            ...
            <div className="flex gap-1.5">
              <input
                ...
                className="flex-1 min-w-0 bg-input-bg rounded px-2 py-1.5 text-xs focus:outline-none focus:ring-1 focus:ring-accent placeholder:text-zinc-400 text-foreground"
              />
              <button onClick={addFriend} disabled={sendingFriend || !newFriendName.trim()} className="px-2 py-1.5 bg-accent hover:bg-accent-hover disabled:opacity-50 rounded text-white shrink-0" title="Adicionar amigo">
                <UserPlus className="w-4 h-4 text-white" />
              </button>
            </div>
            <div className="relative">
              <Search className="w-3.5 h-3.5 absolute left-2 top-1/2 -translate-y-1/2 text-zinc-400" />
              <input value={friendQuery} onChange={(e) => setFriendQuery(e.target.value)} placeholder="Buscar amigo" className="w-full bg-input-bg rounded pl-7 pr-2 py-1.5 text-xs focus:outline-none placeholder:text-zinc-400 text-foreground" />
            </div>
            {incomingRequests.length > 0 && (
              <div>
                <p className="text-[11px] font-bold text-zinc-400 px-1 mb-1">PEDIDOS — {incomingRequests.length}</p>
                {incomingRequests.map((r) => (
                  <div key={r.from_user} className="flex items-center gap-2 px-1.5 py-1.5 rounded bg-surface mb-1">
                    <button onClick={() => onViewProfile(r.from_user)} title="Ver perfil"><Avatar src={r.avatar} name={r.username} className="w-7 h-7 rounded-full bg-surface-active text-xs" /></button>
                    <span className="flex-1 min-w-0 text-xs font-medium text-foreground truncate">{r.username}</span>
                    <button onClick={() => onAcceptFriend(r.from_user)} className="w-7 h-7 rounded-full bg-success hover:bg-success-hover flex items-center justify-center shrink-0" title="Aceitar"><Check className="w-3.5 h-3.5 text-white" /></button>
                    <button onClick={() => onRejectFriend(r.from_user)} className="w-7 h-7 rounded-full bg-surface-hover hover:bg-danger flex items-center justify-center shrink-0" title="Recusar"><X className="w-3.5 h-3.5 text-zinc-300" /></button>
                  </div>
                ))}
              </div>
            )}
            {outgoingRequests.length > 0 && (
              <div>
                ...
                {outgoingRequests.map((r) => (
                  <div key={r.to_user} className="flex items-center gap-2 px-1.5 py-1 rounded hover:bg-surface-hover">
                    <Avatar src={r.avatar} name={r.username} className="w-6 h-6 rounded-full bg-surface-active text-[10px]" />
            ...
            <div>
              ...
              {shownFriends.map((f) => (
                <div key={f.user_id} className="flex items-center gap-2 px-1.5 py-1.5 rounded hover:bg-surface-hover group">
                  <button onClick={() => onViewProfile(f.user_id)} className="relative shrink-0" title="Ver perfil">
                    <Avatar src={f.avatar} name={f.username} className="w-7 h-7 rounded-full bg-surface-active text-xs" />
                    <span className={`absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full border-2 border-surface ${onlineIds.has(f.user_id) ? "bg-success" : "bg-zinc-600"}`} />
                  </button>
                  <button onClick={() => onFriendDM(f.user_id)} className="flex-1 min-w-0 text-left">
                    <span className="block text-xs font-medium text-foreground truncate">{f.username}</span>
                    <span className="block text-[10px] text-zinc-400">{onlineIds.has(f.user_id) ? "Online" : "Offline"}</span>
                  </button>
                  <button onClick={() => onFriendDM(f.user_id)} className="p-1.5 hover:bg-accent rounded opacity-0 group-hover:opacity-100 shrink-0" title="Conversar"><MessageCircle className="w-3.5 h-3.5 text-zinc-300" /></button>
                  <button onClick={() => onRemoveFriend(f.user_id, f.username)} className="p-1.5 hover:bg-danger rounded opacity-0 group-hover:opacity-100 shrink-0" title="Remover"><UserX className="w-3.5 h-3.5 text-zinc-400 hover:text-foreground" /></button>
                </div>
              ))}
            </div>
          </div>
        ...
        <>
          <div className="h-12 px-4 flex items-center justify-between border-b border-border-strong shadow-sm shrink-0">
            <div className="flex items-center gap-2 min-w-0">
              <button onClick={() => setShowMobileSidebar(false)} className="lg:hidden p-1 hover:bg-surface-active rounded mr-1"><X className="w-4 h-4" /></button>
              ...
              <div className="flex items-center gap-1">
                <span className={`text-[10px] px-2 py-0.5 rounded-full ${connected ? "bg-success text-white" : "bg-zinc-600 text-zinc-300"}`}>{connected ? "● AO VIVO" : "offline"}</span>
                {currentServer && <button onClick={() => useModalStore.getState().openModal("showMembersModal")} className="p-1 hover:bg-surface-active rounded" title="Membros e convites"><Users className="w-3.5 h-3.5 text-zinc-400 hover:text-foreground" /></button>}
                {currentServer && canManage && <button onClick={() => useModalStore.getState().openModal("showRolesModal")} className="p-1 hover:bg-surface-active rounded" title="Cargos e permissões"><Shield className="w-3.5 h-3.5 text-zinc-400 hover:text-foreground" /></button>}
                {currentServer && canManage && <button onClick={() => useModalStore.getState().openModal("showWebhooksModal")} className="p-1 hover:bg-surface-active rounded" title="Webhooks"><Webhook className="w-3.5 h-3.5 text-zinc-400 hover:text-foreground" /></button>}
                {currentServer && canManage && <button onClick={() => openEditServer(currentServer)} className="p-1 hover:bg-surface-active rounded" title="Editar servidor"><Settings className="w-3.5 h-3.5 text-zinc-400 hover:text-foreground" /></button>}
                {currentServer && canManage && <button onClick={deleteServer} className="p-1 hover:bg-surface-active rounded" title="Excluir servidor"><Trash2 className="w-3.5 h-3.5 text-zinc-400 hover:text-red-400" /></button>}
                {currentServer && !canManage && <button onClick={onLeaveServer} className="p-1 hover:bg-surface-active rounded" title="Sair do servidor"><DoorOpen className="w-3.5 h-3.5 text-zinc-400 hover:text-red-400" /></button>}
              </div>
          </div>
          <div className="flex-1 overflow-y-auto p-2 space-y-4">
            <div>
                <div className="flex items-center justify-between px-1 py-1 text-xs font-semibold text-zinc-400 tracking-wide">
                  ...
                  {canManage && <Plus onClick={createChannel} className="w-3.5 h-3.5 cursor-pointer hover:text-foreground" />}
                </div>
      ...
      <div className="h-[52px] bg-surface flex items-center px-2 gap-2 shrink-0 relative">
      {voiceStatus.joined && (
        <div className="absolute bottom-full left-0 right-0 bg-input-bg border-t border-black/40 px-2 py-1.5">
          ...
          <div className="flex items-center gap-1 mt-1">
            <button onClick={() => voiceControls.current?.toggleMute()} className={`flex-1 py-1 rounded text-[11px] font-semibold ${voiceStatus.muted ? "bg-danger text-white" : "bg-surface hover:bg-surface-hover text-foreground"}`} title={voiceStatus.muted ? "Ativar microfone" : "Mutar"}>
              {voiceStatus.muted ? "Mutado" : "Mutar"}
            </button>
            <button onClick={() => voiceControls.current?.toggleDeafen()} className={`flex-1 py-1 rounded text-[11px] font-semibold ${voiceStatus.deafened ? "bg-danger text-white" : "bg-surface hover:bg-surface-hover text-foreground"}`} title="Surdo">
              {voiceStatus.deafened ? "Ensurdecido" : "Ensurdecer"}
            </button>
            <button onClick={() => voiceControls.current?.leave()} className="flex-1 py-1 rounded text-[11px] font-semibold bg-danger hover:bg-danger-hover text-white" title="Sair da voz">
              Sair
            </button>
          </div>
        </div>
        ...
        {showStatusMenu && (
          <div className="absolute bottom-full left-2 mb-2 w-52 bg-surface border border-input-bg rounded-lg shadow-xl overflow-hidden z-50">
            ...
            {isDesktop && ptt && (
              <div className="border-t border-input-bg px-3 py-2">
                <button
                  onClick={() => applyPtt({ ...ptt, enabled: !ptt.enabled })}
                  className="w-full flex items-center gap-2 text-sm text-foreground"
                >
                  <span className={`w-3 h-3 rounded-full ${ptt.enabled ? "bg-success" : "bg-zinc-600"}`} />
                  Push-to-talk {ptt.enabled ? "ligado" : "desligado"}
                </button>
                {ptt.enabled && (
                  <button
                    onClick={() => setCapturingPtt(true)}
                    className="mt-1.5 w-full text-left text-xs bg-surface hover:bg-surface-hover rounded px-2 py-1.5 text-zinc-300"
                  >
