...
export default function ChannelSidebar(props: Props) {
  ...
  const channelRow = (ch: Channel, icon: React.ReactNode) => (
    <div key={ch.id} className={`group flex items-center gap-1 px-2 py-1 rounded mt-0.5 ${selectedChannel === ch.id ? "bg-surface-active text-white" : "text-zinc-400 hover:bg-surface-hover hover:text-zinc-200"} ${sidebarCollapsed ? "justify-center" : ""}`} title={sidebarCollapsed ? ch.name : undefined}>
      ...
      {viewMode !== "server" ? (
        <>
          ...
          <div className="h-12 px-3 flex items-center gap-1 border-b border-[#1F2124] shadow-sm shrink-0">
            <button onClick={() => setSideTab("dms")} className={`flex-1 py-1.5 rounded text-[13px] font-semibold transition-colors ${sideTab === "dms" ? "bg-surface-active text-white" : "text-zinc-400 hover:text-zinc-200"}`}>Conversas</button>
            <button onClick={() => setSideTab("friends")} className={`flex-1 py-1.5 rounded text-[13px] font-semibold transition-colors flex items-center justify-center gap-1.5 ${sideTab === "friends" ? "bg-surface-active text-white" : "text-zinc-400 hover:text-zinc-200"}`}>
              Amigos
          ...
          <div className="flex-1 overflow-y-auto p-2 space-y-1">
            {filteredDMs.length === 0 ? (
              ...
              <button key={dm.id} onClick={() => { setSelectedDM(dm.id); setViewMode("dm"); }} className={`w-full flex items-center gap-3 px-2 py-2 rounded text-left ${selectedDM === dm.id ? "bg-surface-active text-white" : "text-zinc-400 hover:bg-surface-hover hover:text-zinc-200"} ${sidebarCollapsed ? "justify-center" : ""}`} title={sidebarCollapsed ? (dm.otherUser?.username || "DM") : undefined}>
          ...
          <div className="flex-1 overflow-y-auto p-2 space-y-2">
            ...
            <div className="flex gap-1.5">
              <input
                ...
                className="flex-1 min-w-0 bg-input-bg rounded px-2 py-1.5 text-xs focus:outline-none focus:ring-1 focus:ring-accent placeholder:text-zinc-400 text-zinc-200"
              />
              <button onClick={addFriend} disabled={sendingFriend || !newFriendName.trim()} className="px-2 py-1.5 bg-accent hover:bg-accent-hover disabled:opacity-50 rounded text-zinc-200 shrink-0" title="Adicionar amigo">
                <UserPlus className="w-4 h-4 text-white" />
              </button>
            </div>
            <div className="relative">
              <Search className="w-3.5 h-3.5 absolute left-2 top-1/2 -translate-y-1/2 text-zinc-400" />
              <input value={friendQuery} onChange={(e) => setFriendQuery(e.target.value)} placeholder="Buscar amigo" className="w-full bg-input-bg rounded pl-7 pr-2 py-1.5 text-xs focus:outline-none placeholder:text-zinc-400 text-zinc-200" />
            </div>
              ...
              <div>
                ...
                {incomingRequests.map((r) => (
                  <div key={r.from_user} className="flex items-center gap-2 px-1.5 py-1.5 rounded bg-[#232428] mb-1">
                    ...
                    <button onClick={() => onRejectFriend(r.from_user)} className="w-7 h-7 rounded-full bg-surface-hover hover:bg-[#DA373C] flex items-center justify-center shrink-0" title="Recusar"><X className="w-3.5 h-3.5 text-zinc-300" /></button>
                  </div>
                ))}
              </div>
            )}
          ...
          <div className="h-12 px-4 flex items-center justify-between border-b border-[#1F2124] shadow-sm shrink-0">
            <div className="flex items-center gap-2 min-w-0">
              ...
              <div className="flex items-center gap-1">
                ...
                {currentServer && <button onClick={() => useModalStore.getState().openModal("showMembersModal")} className="p-1 hover:bg-surface-active rounded" title="Membros e convites"><Users className="w-3.5 h-3.5 text-zinc-400 hover:text-white" /></button>}
                {currentServer && canManage && <button onClick={() => useModalStore.getState().openModal("showRolesModal")} className="p-1 hover:bg-surface-active rounded" title="Cargos e permissões"><Shield className="w-3.5 h-3.5 text-zinc-400 hover:text-white" /></button>}
                {currentServer && canManage && <button onClick={() => useModalStore.getState().openModal("showWebhooksModal")} className="p-1 hover:bg-surface-active rounded" title="Webhooks"><Webhook className="w-3.5 h-3.5 text-zinc-400 hover:text-white" /></button>}
                {currentServer && canManage && <button onClick={() => openEditServer(currentServer)} className="p-1 hover:bg-surface-active rounded" title="Editar servidor"><Settings className="w-3.5 h-3.5 text-zinc-400 hover:text-white" /></button>}
      ...
      {voiceStatus.joined && (
        <div className="absolute bottom-full left-0 right-0 bg-input-bg border-t border-black/40 px-2 py-1.5">
          ...
          <div className="flex items-center gap-1 mt-1">
            <button onClick={() => voiceControls.current?.toggleMute()} className={`flex-1 py-1 rounded text-[11px] font-semibold ${voiceStatus.muted ? "bg-[#DA373C] text-white" : "bg-surface hover:bg-surface-hover text-zinc-200"}`} title={voiceStatus.muted ? "Ativar microfone" : "Mutar"}>
              {voiceStatus.muted ? "Mutado" : "Mutar"}
            </button>
            <button onClick={() => voiceControls.current?.toggleDeafen()} className={`flex-1 py-1 rounded text-[11px] font-semibold ${voiceStatus.deafened ? "bg-[#DA373C] text-white" : "bg-surface hover:bg-surface-hover text-zinc-200"}`} title="Surdo">
        ...
        {showStatusMenu && (
          <div className="absolute bottom-full left-2 mb-2 w-52 bg-[#232428] border border-input-bg rounded-lg shadow-xl overflow-hidden z-50">
