...
export default function MembersSidebar({ showMobileMembers, onlineMembers, allProfiles, status, onViewProfile }: Props) {
  ...
  return (
    <div className={`${showMobileMembers ? "fixed inset-y-0 right-0 translate-x-0" : "hidden"} lg:translate-x-0 lg:relative lg:flex z-50 w-60 bg-[#2B2D31] flex-col shrink-0 overflow-y-auto h-full transition-transform duration-200`}>
      <div className="p-3 space-y-4">
        ...
        {onlineMembers.map((m) => (
          <button key={m.id} onClick={() => onViewProfile(m.id)} className="w-full flex items-center gap-3 px-2 py-1 rounded hover:bg-[#35373C] cursor-pointer group text-left">
            <div className="relative"><Avatar src={m.avatar} name={m.username} className="w-8 h-8 rounded-full bg-[#41434A] text-sm" /><div className={`absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 border-[#2B2D31] ${statusConfig[m.status as keyof typeof statusConfig]?.color || "bg-[#23A559]"}`} /></div>
        ...
        <div className="border-t border-[#3F4147] pt-3 space-y-1">
          ...
          {offline.slice(0, 20).map((m) => (
            <button key={m.id} onClick={() => onViewProfile(m.id)} className="w-full flex items-center gap-3 px-2 py-1 rounded opacity-60 hover:opacity-100 hover:bg-[#35373C] cursor-pointer group text-left">
              <div className="relative"><Avatar src={m.avatar} name={m.username} className="w-8 h-8 rounded-full bg-[#41434A] text-sm grayscale" /><div className="absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 border-[#2B2D31] bg-zinc-500" /></div>
