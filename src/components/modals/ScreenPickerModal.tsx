...
export default function ScreenPickerModal({ sources, onPick, onClose }: Props) {
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
        {list.map((s) => (
          <button
            key={s.id}
            onClick={() => onPick(s.id)}
            className="group rounded-lg overflow-hidden border-2 border-transparent hover:border-accent bg-surface text-left transition"
            title={s.name}
          >
  ...
  return (
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-[80] p-4">
      <div className="bg-background rounded-xl w-full max-w-2xl p-5 shadow-2xl max-h-[85vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-lg font-bold text-white">Compartilhar tela</h2>
          <button onClick={onClose} className="p-1.5 hover:bg-surface-active rounded-full"><X className="w-5 h-5 text-zinc-400" /></button>
        </div>
        {sources === null ? (
