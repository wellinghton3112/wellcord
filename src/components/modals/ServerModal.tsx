      ...
      <div className="p-6">
        ...
        <div className="space-y-4">
          <div>
            ...
            <input value={newServerName} onChange={(e) => setNewServerName(e.target.value)} placeholder="ex: Casa dos Amigos" className="w-full mt-1 bg-surface border border-input-bg rounded px-3 py-2 text-white outline-none focus:border-accent" autoFocus />
          </div>
          <div>
            <label className="text-xs font-bold text-zinc-300 uppercase">Ícone</label>
            <div className="grid grid-cols-8 gap-2 mt-2">
              {ICONS.map((ic) => (
                <button key={ic} onClick={() => { setNewServerIcon(ic); setNewServerImage(null); setNewServerPreview(""); }} className={`w-9 h-9 rounded flex items-center justify-center text-lg border ${newServerIcon === ic && !newServerImage ? "bg-accent border-accent" : "bg-surface border-input-bg hover:bg-surface-active"}`}>{ic}</button>
              ))}
            </div>
          </div>
          <div>
        ...
        <div className="flex justify-end gap-3 mt-6">
          ...
          <button onClick={onSave} disabled={!newServerName.trim() || creatingServer} className="px-6 py-2 bg-accent hover:bg-accent-hover disabled:opacity-50 rounded text-sm font-medium text-white">{creatingServer ? "Salvando..." : editingServer ? "Salvar" : "Criar"}</button>
        </div>
      </div>
    </ModalShell>
  );
